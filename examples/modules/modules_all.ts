...
function handler(req: Request): Response {
  ...
  if (req.method === "POST") {
    ...
    const jwt = jwtSign(
      JSON.stringify({ sub: "user-123", iat: 1700000000 }),
      secret,
    );

    return Response.json(
      { user: validation.value, token: jwt, cache: cacheStats() },
      { status: 201 },
    );
  }

  return Response.json({ schema_compiled: schema_ok, cache: cacheStats() });
}
