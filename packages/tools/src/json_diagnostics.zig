...
pub fn fromUnsupportedSourceExtension(file: []const u8) JsonDiagnostic {
    return .{
        ...
        .line = 1,
        .column = 1,
        .suggestion = "rename the handler to .ts, or use .tsx when JSX lowering is required",
    };
}
...
fn writeMarkdownCell(writer: anytype, value: []const u8) !void {
...
/// Emit the canonical `docs/restrictions-to-proofs.md` from the complete
/// restriction registry. `scripts/update-restrictions-doc.sh` owns the
/// generated file; the docs drift gate checks its row floor and membership.
pub fn writeRestrictionsMarkdown(writer: anytype) !void {
    ...
    for (restrictionCatalog.restrictions()) |entry| {
        const name = entry.v1_feature_name orelse continue;
        try writer.writeAll("| `");
        try writeMarkdownCell(writer, name);
        try writer.writeAll("` | ");
        try writeMarkdownCell(writer, entry.failure_class.?);
        try writer.writeAll(" | ");
        try writeMarkdownCell(writer, entry.proof_unlocked.?);
        try writer.writeAll(" | ");
        try writeMarkdownCell(writer, entry.alternative.?);
        try writer.writeAll(" |\n");
    }
    try writer.writeAll(
        \\
    ...
    for (restrictionCatalog.restrictions()) |entry| {
        ...
        try writer.writeAll("| ");
        try writeMarkdownCell(writer, entry.feature);
        try writer.writeAll(" | ");
        try writeMarkdownCell(writer, entry.boundary);
        try writer.writeAll(" | `");
        try writeMarkdownCell(writer, entry.nature.label());
        try writer.writeAll("` | ");
        if (entry.enforced_by.len == 0) {
            try writer.writeAll("no diagnostic");
        } else {
            for (entry.enforced_by, 0..) |code, index| {
                if (index > 0) try writer.writeAll(", ");
                try writer.writeByte('`');
                try writeMarkdownCell(writer, code);
                try writer.writeByte('`');
            }
        }
        try writer.writeAll(" |\n");
    ...
    try writer.writeByte('\n');
    for (restrictionCatalog.restrictions()) |entry| {
        const note = entry.unenforced_note orelse continue;
        try writer.writeAll("- **");
        try writeMarkdownCell(writer, entry.feature);
        try writer.writeAll("** - ");
        try writeMarkdownCell(writer, note);
        try writer.writeByte('\n');
    }
    try writer.writeAll(
        \\
    ...
    for (restrictionCatalog.restrictions()) |entry| {
        ...
        try writeMarkdownCell(writer, entry.feature);
        try writer.writeAll("** - ");
        try writeMarkdownCell(writer, entry.note);
        try writer.writeByte('\n');
    }
}
...
test "writeRestrictionsMarkdown renders every restriction" {
    const allocator = std.testing.allocator;
    var buf: std.ArrayList(u8) = .empty;
    defer buf.deinit(allocator);
    var aw: std.Io.Writer.Allocating = .fromArrayList(allocator, &buf);
    ...
    for (restrictionCatalog.restrictions()) |entry| {
        ...
        var expected_writer: std.Io.Writer.Allocating = .fromArrayList(allocator, &expected);
        try writeMarkdownCell(&expected_writer.writer, entry.feature);
        expected = expected_writer.toArrayList();
        const found = std.mem.indexOf(u8, buf.items, expected.items) != null;
        try std.testing.expect(found);
    }
    try std.testing.expect(std.mem.indexOf(u8, buf.items, "Do not edit this file manually") != null);
}
...
test "unsupported source extensions have one exact repair" {
    const diagnostic = fromUnsupportedSourceExtension("handler.js");
    try std.testing.expectEqualStrings("ZTS052", diagnostic.code);
    try std.testing.expectEqualStrings("handler.js", diagnostic.file);
