...
- [validate-json-strips-the-label-it-was-asked-to-check](validate-json-strips-the-label-it-was-asked-to-check.md) - the next instance of this class, found and fixed 2026-08-03. `validateJson`, `coerceJson`, and `decodeJson` each labelled their output from their own contract rather than from what they were handed, so any label laundered through them: `env("JWT_SECRET")` routed through one and returned in the body proved `no_secret_leakage`. Read it against the Prevention rule below about fixing one instance and assuming the class was closed - the JSON round-trip was closed here, and these three sat beside it untouched. Note also who found it: a model, during a corpus recording, which wrote the laundering deliberately and said so in its commentary.
