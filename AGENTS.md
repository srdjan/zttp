...
- Adding an advertised rule needs either a defect seed in `packages/pi/src/standin/defect_seeds.zig` that is observed tripping it, or a row in `scripts/unseeded-rules.allow` giving the reason a seed cannot reach it; a row for a code a seed now verifies fails the same gate. Both directions run inside `zig build test-standin`. The list is not a backlog: 17 of its 19 rows name a code the registry advertises and no code path constructs, so no seed and no recorded draft can ever trip them, and the reason belongs beside the row rather than in a reader's assumption about what `docs/coverage.md` leaves out.
