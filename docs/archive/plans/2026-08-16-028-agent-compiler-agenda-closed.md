# Agent-compiler agenda: the closed items

Archived 2026-08-16, lifted verbatim from `docs/roadmap.md`.

Eight of the nine items on this agenda closed. Each one records what it found
rather than only that it shipped, which is why the text is kept instead of a
line saying "done". `docs/roadmap.md` keeps the one open item, the sequencing
note that still binds it, and the refusals.

Counts and file names below were true when each item closed. Read them for why
...
### 6. Idempotence gate and the repair-span fix (done)
