...
- [a-name-inside-an-intersection-weighed-nothing](a-name-inside-an-intersection-weighed-nothing.md) - the unresolved-name bullet in the member-kind list above, as a shipped defect on the intersection side: the target loop discharged a member it could not resolve, so `type AB = A & B` accepted a value missing `B`'s field. Fixed the way this document's `unionMemberSubsumes` was, by applying D1 amendment A1 at one site ahead of the global rule.
