...
- R9. `apply_edit.before` must stop being model-authored or resent. The host must read the current workspace baseline at the effect boundary, bind it by content hash through veto and approval, and fail closed if it cannot obtain or revalidate that baseline. Model-visible retry history keeps the proposed content and diagnostics but does not duplicate a full prior file.
...
- R20. Normal generation usage, summarization usage, provider attempt counts, estimated request components, actual logical input usage, cache reads, and cache writes must remain separately observable. Anthropic logical input is uncached input plus cache-read plus cache-creation input; providers whose reported input already includes cached tokens use that total without addition. Summary usage contributes to cost totals but not to the product metric for normal round trips to a verified edit.
...
- AE9. A representative editing flow moved behind prompt retrieval reaches the same verified outcome, does not increase median normal generation calls, and reduces cumulative `estimated_logical_input_v1` including retrieval and summarization.
...
- KTD3. **Use a versioned conservative estimator plus normalized fresh usage.** The pure estimator is parameterized by explicit model data and counts UTF-8 bytes plus per-item framing. Its pinned corpus must never undercount and has a bounded overestimate. When the last normal request belongs to the same model and projection checkpoint, use normalized logical input usage plus a conservative estimate for trailing entries. Clear it on model change and compaction.
...
- KTD6. **Make the workspace the edit baseline authority.** Remove `before` from the model tool schema and historical retry payload. `prepareEdit` reads and hashes the current file, veto and approval borrow that baseline, and apply revalidates the hash before write. This removes duplicated input and closes the false-baseline class.
...
- `estimated_logical_input_v1` is the sum of U1's versioned estimates for every provider-neutral request snapshot in one flow, including retrieval and standalone summarization requests. Normalized provider-reported logical input remains a separate empirical metric.
...
1. Characterize fixed-prefix, per-round growth, end-to-end `estimated_logical_input_v1`, normalized provider-reported input, normal call counts, and verified outcomes, then introduce pure request accounting without changing request bytes.
...
- **Approach:** Extend existing request checkpoints with size, normalized usage, and projection metadata. Re-record provider fixtures after the request contract stabilizes. Add full-flow cases for threshold, overflow, resume, fork, split turns, large files, veto retries, request rejection, prompt retrieval, and compaction continuation fidelity. Use captured provider-neutral snapshots to compare verified outcomes, cumulative `estimated_logical_input_v1`, normal call counts, and p50/p95/max request sizes against U1's mandatory offline baseline. Assert fixture floors and complete consumption so an empty corpus cannot pass. Run the real local MLX flow when the pinned server is available and record normalized provider-reported prompt-token improvement separately from deterministic harness evidence.
...
Provider-backed empirical recapture is evidence of real token behavior, not a substitute for deterministic assertions. The mandatory regression authority is `estimated_logical_input_v1` over checked-in provider-neutral request snapshots. If credentials or the local server are unavailable, the implementation may not manufacture responses; it must leave normalized provider-reported logical input explicitly unverified while all offline gates remain mandatory.
...
- [ ] The frozen representative corpus preserves all passing verified outcomes, reduces p95 cumulative `estimated_logical_input_v1` by at least 20 percent, and meets the call-count constraints in Success Criteria; normalized provider-reported input is recorded when empirical capture is available.
