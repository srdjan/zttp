//! Version-2 language-feature discovery through the in-process protocol.
...
fn execute(allocator: std.mem.Allocator, args: []const []const u8) anyerror!registry_mod.ToolResult {
    ...
    const projection = try client.invokeForTool(allocator, .{ .operation = .features, .input_json = "{}" });
    return .{ .ok = projection.ok, .llm_text = projection.llm_text };
}

const testing = std.testing;

test "features registry returns its full version-2 envelope" {
    ...
    try expectDiscoveryEnvelope(result, "features", "features");
}

fn expectDiscoveryEnvelope(result: registry_mod.ToolResult, operation: []const u8, payload_field: []const u8) !void {
    ...
    try testing.expect(envelope.get("payload").?.object.get(payload_field) != null);
}
